# Codex-Promax Agent Install Guide

This guide is the source of truth for coding agents that are asked to install Codex-Promax in an existing repository.

## Goal

Set up Codex-Promax end-to-end in the current repository with minimal user effort and without unnecessary prompts.
...
3. Prefer running Codex-Promax with `npx -y codex-promax@latest` so no global install is required unless the environment clearly requires a different approach.
4. Run:
...
10. Finish with a short summary that states:
    - whether Codex-Promax was initialized successfully
